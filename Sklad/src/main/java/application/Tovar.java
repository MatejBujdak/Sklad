...
public class Tovar extends HttpServlet {     
    ...
    //metoda na vypis tovaru z tabulky tovar
	 protected void vypisTovar(Statement stmt, int id, String operacia) {
	    	try {
	    		ResultSet resultSet = stmt.executeQuery("SELECT * FROM tovar");
	            out.println("<html><body>");
	            out.println("<h1>Zoznam tovaru:</h1>");
	            out.println("<table border='1'>");
	            out.println("<tr><th>id</th><th>nazov</th><th>cena</th></tr>");
	            ...
	            while (resultSet.next()) {
	                ...
	                if(resultSet.getInt("id") == id && operacia.equals("editovat")) {
	                	out.println("<form action='/Sklad/Tovar' method='post'>");
	                	...
	                	out.println("<td><input type='text' name='cena' value='"+ resultSet.getDouble("cena") +"'></td>"); 
		                ...
		                out.println("<td><form action='/Sklad/Tovar' method='post'>");
	 ...
	 protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		  ...
		  if (operacia.equals("odstranit")) {
			  ...
			  pridajPolozku(request.getParameter("nazov"), request.getParameter("cena"), out);
	 ...
	 // UPDATE
	 private void upravPolozku(PrintWriter out, String id, String nazov, String cena) {
		 try {
		        Statement stmt = con.createStatement();
		        String sql = "UPDATE tovar SET nazov = '" + nazov + "', cena = " + cena + " WHERE id = " + id;
	 ...
	 private void vymazPolozku(PrintWriter out, String id) {
		 try {
		        ...
		        ResultSet checkResult = checkStmt.executeQuery("SELECT COUNT(*) AS count FROM distribucie WHERE tovar_id = " + id);

		        if (checkResult.next() && checkResult.getInt("count") > 0) {
		            out.println("Nie je možné vymazať tovar, pretože existujú odkazy na tento záznam v spojovacej tabuľke.");
		        } else {
		            Statement stmt = con.createStatement();
		            String sql = "DELETE FROM tovar WHERE id = " + id;
		            stmt.executeUpdate(sql);
		            String sql2 = "DELETE FROM distribucie WHERE tovar_id = " + id;
	 ...
	 // INSERT
	 private void pridajPolozku(String nazov, String cena, PrintWriter out) {
		    try {
		    	Statement stmt = con.createStatement();
		        if (nazov.isEmpty() || cena.isEmpty()) {
		            out.println("Nesmu byt prazdne udaje.");
		            return;
		        }

		        Double cenaValue = Double.parseDouble(cena);

		        String sql = "INSERT INTO tovar (nazov, cena) VALUES (";
		        sql += "'" + nazov + "', ";
		        sql += "'" + cenaValue + "') ";
		        stmt.executeUpdate(sql);
